    ...
    <section id="contact" className="px-8 py-[50px]" style={{ borderBottom: '3px solid var(--color-ink)' }}>
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
        ...
        <h2 className="text-[52px] font-black leading-[0.95] tracking-[-2px]" style={{ color: 'var(--color-ink)' }}>
          {lang === 'zh' ? (
            <>
              来<span className="border-[3px] px-[5px]" style={{ background: 'var(--color-accent-cyan)', borderColor: 'var(--color-ink)' }}>聊</span><br />
              一下？
            </>
          ) : (
            <>
              Let's<br />
              <span className="border-[3px] px-[5px]" style={{ background: 'var(--color-accent-cyan)', borderColor: 'var(--color-ink)' }}>Chat</span>?
            </>
          )}
        </h2>

        <div className="flex flex-col gap-3">
          <a href="mailto:alexeisiemail@gmail.com"
          ...
          <a href="http://xiaohongshu.com/user/profile/692823cc000000003201cfdd"
          ...
          <a href="https://wpa.qq.com/msgrd?v=3&uin=815530704&site=qq&menu=yes"
            target="_blank" rel="noopener noreferrer"
            className="border-[2.5px] px-6 py-3 font-mono text-[12px] font-black uppercase tracking-[1.5px] transition-transform active:scale-[0.97]"
