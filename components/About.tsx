...
import { STACK, t } from '@/lib/content'
    ...
    <section id="about" className="px-8 py-[50px]" style={{ borderBottom: '3px solid var(--color-ink)' }}>
      ...
      <div className="grid gap-7 md:grid-cols-2">
        <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
          ...
          className="text-[14px] leading-[1.75] opacity-65" style={{ color: 'var(--color-ink)' }}>
          {lang === 'zh' ? (
            <>
            ...
            <>
              <strong className="font-black opacity-100">Sanli</strong>, HUST undergrad.
              Joining <strong className="font-black opacity-100">ByteDance</strong> as a frontend engineer in July 2026.
              <br /><br />
              Day job: writing code. After hours: building things. Believes AI is redefining the ceiling for indie developers.
              <br /><br />
              Currently exploring an AI-powered path to independence.
            </>
          )}
        </motion.p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
          ...
          {STACK.map(({ dot, key, value }) => (
            <div key={key.en} className="flex items-center gap-3 border-[2px] px-[14px] py-[11px]"
              ...
              <span className="font-mono text-[11px] font-black uppercase tracking-[1px]" style={{ color: 'var(--color-ink)' }}>{t(key, lang)}</span>
